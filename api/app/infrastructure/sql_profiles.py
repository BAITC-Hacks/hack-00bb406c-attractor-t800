...
class SqlProfileRepository(ProfileRepository):
    ...
    def get_profile(self, employee_id: str) -> dict:
        ...
        skill_names = {skill.skill_id: skill.name for skill in self.db.query(Skill).all()}
        ...
        return {
            ...
            "history": [{"record_id": record.record_id, "event_id": event.event_id, "title": event.title, "date": record.date, "status": record.status, "completed_at": record.completed_at, "recorded_at": record.recorded_at, "completion_pct": record.completion_pct, "score": record.score, "mandatory": event.mandatory, "format": event.format, "develops_skills": event.develops_skills} for record, event in history],
            "assessed_skills": dict(employee.skills),
            "role_profiles": [{"role": p.role, "grade": p.grade, "required_skills": p.required_skills, "critical_skills": p.critical_skills} for p in self.db.query(RoleProfile).all()],
