...
class SqlProfileRepository(ProfileRepository):
    ...
    def get_profile(self, employee_id: str) -> dict:
        ...
        return {
            ...
            "history": [{"record_id": record.record_id, "event_id": event.event_id, "title": event.title, "date": record.date, "status": record.status, "completion_pct": record.completion_pct, "score": record.score, "mandatory": event.mandatory, "format": event.format, "develops_skills": event.develops_skills} for record, event in history],
